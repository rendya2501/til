...
-r :: ハードウェアクロックを表示する。デフォルト動作なので省略可能。
...
Stratum：ソースのストレイタム（階層）。原子時計やGPSなどの正確な時計をstratum 0とし、そこから直接時刻を取得するサーバがstratum 1、stratum 1のサーバから時刻を取得するサーバはstratum 2、最下層は stratum 16（stratum 16のサーバからは時刻を取得することはできない）
