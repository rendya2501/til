...
久しぶりにやったけど、混乱するものは混乱するわ。  
話はわかるんだけど今回は落としきれなかった感じ。  
プログラミングの問題って完全にわからない時以外は解説が必要ない気がする。  
今回は何回もやってるから別にいいけど、もっとノートを使って取り組めばよかったかもなぁ。  
まぁ、それでも7割取れてるから、俺からしたら十分ではあるけどねぇ。  

基本情報やったときにさんざんアルゴリズムとJavaをやったおかげで、前のやり方を適応しつつやることが出来ている点はいいかな。  
...
否定論理和
論理和が線形問題であり、その否定だから 100%
他)否定論理和は線形問題だから

---

[2]
図5より、中間ノードm2の出力値は以下の演算式で求めることができます。
m2 = 0 (x1 × w1(m2) + x2 × w2(m2) + b ≦ 0の場合)
m2 = 1 (x1 × w1(m2) + x2 × w2(m2) + b > 0の場合)
図5中で言えば以下の2つのアーク及びバイアスを合計する処理になります。
pm03_9.gif/image-size:228×138
演算式に、問題文のx1=1、x2=0、表3のw1(m2)=-0.5、m2(w2)=-0.5、b=0.7を、それぞれ代入すると、
...
→
0.2まで求めて安心してそのまま答えにしてしまった。
中間ノードも判定してTFを決めるロジックに気が付いていればこんなことは無かっただろう。
...
X[0][0] × WY[0] ⇔ 0 × 0.5
X[0][1] × WY[1] ⇔ 0 × 0.5
というように「入力値×重み」を計算できます。よって、[イ]には入力値を表す X[out][in] が、[ウ]には重みを表す WY[in] が、それぞれ入ります。

∴イ＝X[out][in]
　ウ＝WY[in]

〔オについて〕
3層パーセプトロンのプログラムでは、単純パーセプトロンから以下の部分が変更されています。
pm03_11.gif/image-size:532×413
この部分は入力値から各中間ノードの出力を求め、その出力値をもとに ytemp に値を加算する処理を行っています。単純パーセプトロンでは入力値にノードyのアークが持つ重み WY を乗じていましたが、3層構造では入力ノードは中間ノードに連結されているので、入力値に乗じるのは中間ノードのアークが持つ重みになります。
...
m1 × w1(y) + m2 × w2(y) + b(y)
ここでは、各中間ノードの出力値にノードyのアークが持つ重みを乗じて演算結果を求めています。各中間ノードの出力値は直前の分岐処理で M[out][mid] に格納されているので、これをそのまま利用できます。よって、[カ]には M[out][mid] が入ります。

重みにはノードyのアークが持つ重みである WY を使用します。演算式では「m1 × w1(y)」「m2 × w2(y)」というように、1番目の中間ノードには1番目の重みを、2番目の中間ノードには2番目の重みを使用しているので、M[out][mid] のときには WY[mid] を指定するのが適切です。
...
[4]
問題文の〔単純パーセプトロンを用いた論理演算〕に、「単純パーセプトロンに適切な重みとバイアスを設定することで、論理積、論理和及び否定論理積を含む線形問題を計算する分類器を構成することができる」とあります。このことから、線形問題は単純パーセプトロンで解くことができることが読み取れます。

同値(EQ)と否定論理和(NOR)が線形問題であるかどうかを判断するために、図2「2入力の論理演算を分類機によって解いた例」のような図に表します。すると、同値(EQ)は1本の直線で分類できず、否定論理和(NOR)は1本の直線で分類できることがわかります。
pm03_10.gif/image-size:404×225
...
## 情報システム開発 道路交通信号機の状態遷移設計

信号機の問題だ。良ーく覚えてる。  
元年の中では出来たほうで得点源になったはず。  
当時はごちゃごちゃしてて冷静ではいられなかったけど、今は普通に正解できる。  
これは間違ったところ以外の復習はいらないな。  
...
主道路信号の状態がC-4になった場合に歩行者信号が青になってしまう 100%
他) ・状態C-5のときに，主道路信号が右折可能で，歩行者信号が青になる (32文字)
    ・状態C-4でR3の遷移が可能となり主道路信号が右折可能で，歩行者信号が青になる (39文字)
イ
...
[1][2]
ボタン押下を契機に開始する処理は、表2のP1及び表3のR2だけです。
主道路信号が黄色のとき、主道路信号の状態はC-6、歩行者信号の状態はW-1ですから、処理前状態がW-1であることを条件とするR2の処理だけが発火します。

主信号が黄になった直後に、ボタン押下があると以下のように遷移していきます。
0. S6の処理が、主道路信号を黄にし、タイマ3(3秒)をセットする。
1. 0.直後のボタン押下でR2が発火し、歩行者信号が主道路信号の監視を開始する。歩行者信号の状態はW-1からW-2に遷移する。
2. タイマ3(3秒)が終了し、S7が発火する。主道路信号を赤にし、タイマ1(15秒)をセットする。主道路信号の状態はC-6からC-1に遷移する。
3. 3.で主道路信号の状態がC-1になったので、R3が発火する。主道路信号の状態監視を終了して、タイマ7(3秒)をセットする。
4 . タイマ7が終了し、R4が発火する。歩行者信号を青にし、タイマ8(8秒)をセットする。
この流れを図で表すと次のようになります。

→
あー、そういうことか。
主道路の図ばかり見ていたけど、見るべきは歩行者側で、歩行者側はメインがC-1に移行した瞬間に3秒のタイマーをセットして、その後、歩行者信号を青にする。
その間C-1継続でメインの信号は赤のままだから歩行者は安全に渡れるってわけか。
だから最小の時間はC-1に遷移するまでの3秒と遷移した後の歩行者側の3秒の合わせて6秒というわけね。
```
