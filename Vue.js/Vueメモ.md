...
`C:\Develop\~~~\WebContent`ならOK。  
`C:\VSSTEMP\~~~\WebContent`だとNG。  
...
`[Authorize(AuthenticationSchemes = OtherSystemCustomerAuthenticationOptions.DEFAULT_SCHEME, Roles = nameof(WebCoopClsType.ReferrerName))]`  

結論からいうとヘッダーに以下の指定をすればよかった。  

``` txt
coop-cls-type-name : ReferrerName
...
const routes: Array<RouteConfig> = [
  {
    ...
    beforeEnter: (to, from, next) => {
      ...
      to.query.ReferrerName = 'ReferrerName';
